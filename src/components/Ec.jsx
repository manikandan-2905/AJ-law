...
const Ec = () => {
  ...
  const handleAddNew = () => {
    ...
    setFormData({ date: "", ecNo: "", vendor: "", customerName: "", office: "", nagar: "", surveyNo: "", amount: "", commission: "", others: "", totalFee: "", received: "", balance: "" });
    setShowModal(true);
  };
  const handleEdit = (ec, e) => {
    e.stopPropagation(); setIsEdit(true); setCurrentEC(ec);
    setFormData({ date: ec.date, ecNo: ec.ecNo, vendor: ec.vendor, customerName: ec.customerName, office: ec.office, nagar: ec.nagar, surveyNo: ec.surveyNo, amount: ec.amount.toString(), commission: ec.commission.toString(), others: ec.others.toString(), totalFee: ec.totalFee.toString(), received: ec.received.toString(), balance: ec.balance.toString() });
    setShowModal(true);
  };
  const handleView = (ec) => { setCurrentEC(ec); setShowViewModal(true); };
  const handleDelete = (id, e) => { e.stopPropagation(); if (window.confirm("Are you sure you want to delete this record?")) { setEcData(ecData.filter(d => d.id !== id)); } };
  ...
  const handleSave = () => {
    ...
    const newEC = { ...formData, id: isEdit ? currentEC.id : Date.now(), totalFee, received, balance, status };
    if (isEdit) { setEcData(ecData.map(d => d.id === currentEC.id ? newEC : d)); } else { setEcData([...ecData, newEC]); }
    setShowModal(false);
  };
